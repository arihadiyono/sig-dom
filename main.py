...
def main_app():
    ...
    if menu == "🗺️ Peta Wilayah Antaran":
        ...
        try:
            with engine.connect() as conn:
                # Mengambil geom, info wilayah, dan hitung LUAS AREA (km2)
                df = pd.read_sql(text("""
                    SELECT 
                        kodepos, kecamatan, kelurahan, 
                ...
                for _, row in df.iterrows():
                    ...
                    folium.GeoJson(
                        ...
                        style_function=lambda x, color=color: {
                            'fillColor': color, 'color': 'white', 'weight': 2, 'fillOpacity': 0.7
                        },
                        tooltip=folium.Tooltip(f"Kodepos: {row['kodepos']}<br>Kelurahan: {row['kelurahan']}<br>Luas: {row['luas_km2']:.2f} km²")
                    ).add_to(m)
                ...
                for idx, row in df.iterrows():
                    with cols[idx % 5]:
                        ...
                        st.markdown(f"""
                            ...
                            <div style="text-align:center; font-size:12px; margin-top:5px; line-height: 1.2;">
                                <b>{row['kelurahan']}</b><br>
                                <span style="color:gray;">{row['kecamatan']}</span><br>
                                <span style="color:#003366; font-weight:bold;">{row['luas_km2']:.2f} km²</span>
                            </div>
                            <br>
    ...
    # --- KONTEN MENU: RIWAYAT ANTARAN (Script sebelumnya tetap sama) ---
            ...
            with engine.connect() as conn:
                ...
                if dict_petugas:
                    ...
                    if not df_titik.empty:
                        ...
                        st_folium(m_antaran, width="100%", height=400)
