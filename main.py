                ...
                if dict_petugas:
                    ...
                    if not df_titik.empty:
                        ...
                        for _, row in df_titik.iterrows():
                            ...
                            folium.Marker([row['latitude'], row['longitude']], icon=folium.Icon(color=color_icon, icon='bicycle', prefix='fa')).add_to(m_antaran)
                        ...
                        # Kalkulasi Prosentase
                        p_berhasil = (berhasil_k / total_k * 100) if total_k > 0 else 0
                        p_gagal = (gagal_k / total_k * 100) if total_k > 0 else 0

                        m1, m2, m3 = st.columns(3)
                        m1.metric("Total Kiriman", total_k)
                        m2.metric("Berhasil (DELIVERED)", f"{berhasil_k}", f"{p_berhasil:.1f}%")
                        m3.metric("Gagal (FAILED)", f"{gagal_k}", f"-{p_gagal:.1f}%", delta_color="inverse")
